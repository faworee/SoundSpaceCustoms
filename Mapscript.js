    ...
    document.querySelectorAll('.custom-dropdown').forEach(dropdown => {
        ...
        header.addEventListener('click', (e) => {
            ...
            document.querySelectorAll('.dropdown-options').forEach(opt => {
                if (opt !== options) {
                    ...
                    opt.previousElementSibling.querySelector('.dropdown-arrow').classList.remove('rotated');
                }
            });
        ...
        dropdown.querySelectorAll('.dropdown-option').forEach(option => {
            option.addEventListener('click', () => {
                ...
                if (dropdown.querySelector('#primarySortText')) {
                    if (typeof handlePrimarySort !== 'undefined') {
                        handlePrimarySort(value);
                    }
                } else if (dropdown.querySelector('#secondarySortText')) {
                    if (typeof handleSecondarySort !== 'undefined') {
                        handleSecondarySort(value);
                    }
                } else if (dropdown.querySelector('#searchTypeText')) {
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.custom-dropdown')) {
            document.querySelectorAll('.dropdown-options').forEach(options => {
                ...
                options.previousElementSibling.querySelector('.dropdown-arrow').classList.remove('rotated');
            });
        }
    });
}
...
(function() {
    ...
    if (!fontDropdownHeader || !fontOptionsList || !selectedFontName || 
        !customFontInputs || !customFontUrl || !customFontName || !applyCustomFont) {
        return;
    }
    ...
    const fontTabContents = document.querySelectorAll('.font-tab-content');
    
    fontTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            fontTabs.forEach(t => t.classList.remove('active'));
            fontTabContents.forEach(c => c.classList.remove('active'));
    ...
    if (fontFileUpload) {
        fontFileUpload.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                if (!uploadedFontName.value) {
                    uploadedFontName.value = fontNameFromFile;
                }
            }
        });
    }
            ...
            try {
                ...
                reader.onload = async (e) => {
                    ...
                    try {
                        ...
                        console.warn('Could not save uploaded font to localStorage (file may be too large)');
                    }
    ...
    function loadSavedFont() {
        ...
        if (uploadedFontData && uploadedFontNameStored) {
            ...
            style.textContent = `
                @font-face {
                    font-family: '${uploadedFontNameStored}';
                    src: url('${uploadedFontData}') format('${uploadedFontFormat}');
                    font-weight: normal;
                    font-style: normal;
                }
            `;
    ...
    async function loadCustomFont(url, fontName) {
        ...
        if (url.includes('fonts.google.com/specimen/')) {
            ...
            if (match) {
                ...
                if (!extractedFontName) {
                    extractedFontName = match[1].replace(/\+/g, ' ');
                }
            }
            ...
            if (family) {
                ...
                if (!extractedFontName) {
                    extractedFontName = family.split(':')[0].replace(/\+/g, ' ');
                }
            }
            ...
            cssUrl = url;
            if (!extractedFontName) {
                alert('For Adobe Fonts, please provide the font family name in the second field.');
                return;
            }
        } else if (url.endsWith('.css') || url.includes('fonts') || url.includes('css')) {
            cssUrl = url;
            if (!extractedFontName) {
                alert('Please provide the font family name for custom font URLs.');
                return;
            }
        ...
        try {
            ...
            if (extractedFontName && extractedFontName !== fontName) {
                setCookie('customFontName', extractedFontName);
            }
        } catch (error) {
            document.body.style.fontFamily = `"${extractedFontName}", sans-serif`;
        }
...
(function() {
    ...
    function init() {
        ...
        if (shareYesBtn) {
            shareYesBtn.addEventListener('click', () => {
                document.getElementById('usernameContainer').style.display = 'block';
            });
        }
        
        if (shareNoBtn) {
            shareNoBtn.addEventListener('click', async () => {
...
if (difficultyFilterBtn) {
    difficultyFilterBtn.addEventListener('click', () => {
        document.getElementById('difficultyModal').classList.add('show');
    });
}

if (patternsFilterBtn) {
    patternsFilterBtn.addEventListener('click', () => {
...
if (otherFilterBtn) {
    otherFilterBtn.addEventListener('click', () => {
...
try {
    ...
    if (savedPreference === 'true' && qualityToggle) {
        qualityToggle.click();
    }
} catch (e) {}
...
window.openEditMap = function(button, event) {
    ...
    try {
        ...
        document.querySelectorAll('#editDifficultyOptions .filter-option').forEach(btn => {
            ...
            if (btn.dataset.difficulty === mapData.difficulty) {
                btn.classList.add('active');
            }
        });
...
document.getElementById('saveMapEditsBtn')?.addEventListener('click', async function() {
    ...
    const user = window.getDiscordUser();
    if (!user) {
        alert('Please login with Discord to edit maps');
        return;
    }
    
    if (user.id !== currentEditingMap.discordId && !isAdmin()) {
        alert('You can only edit your own maps');
        return;
    }
    ...
    const selectedPatterns = Array.from(document.querySelectorAll('#editPatternOptions .filter-option.active'))
        .map(btn => btn.dataset.pattern);
    ...
    currentEditingMap.patterns = selectedPatterns.length > 0 ? selectedPatterns : ["No Data"];
...
document.getElementById('deleteMapBtn')?.addEventListener('click', async function() {
    if (!isAdmin()) {
        alert('Only admins can delete maps');
        return;
    }
    ...
    try {
        const response = await fetch(MAP_API_URL, {
            ...
            body: JSON.stringify({
                ...
                userId: user.id,
                isAdmin: user.isAdmin,
                mapLink: currentEditingMap.link
            })
        });
...
function createMapCard(m) {
    ...
    ? `<div class="dropdown-action" onclick="openEditMap(this, event)">Edit Map</div>` 
    : ''; 
    card.innerHTML = `
        ${newTagHtml}
...
async function loadMapData() {
    try {
        ...
        const mapPromises = mapData.map(async (mapArray) => {
            ...
            const [link, mapName, artist, mapper, difficulty, patterns, isNew, starRating, info, discordId] = mapArray;
...
async function loadMapCopies() {
    try {
        const timestamp = Date.now();
        const response = await fetch('https://faworee.com/soundspacecustoms/get-mapcopies.php?v=' + timestamp);
